//! Equivalent two-pass textured cube through ordinary `wgpu` and `winit` APIs.
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attributes = Window::default_attributes()
            .with_title("wgpu — postprocessed textured cube")
        ...
        match event_loop.create_window(attributes) {
            Ok(window) => {
                ...
                match pollster::block_on(Gpu::new(window.clone())) {
                    Ok(gpu) => {
                        println!("backend: wgpu ({:?}), samples: Four", gpu.backend);
                        self.gpu = Some(gpu);
...
impl Gpu {
    async fn new(window: Arc<Window>) -> Result<Self, Box<dyn Error>> {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("postprocess cube shader"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("postprocess cube pipeline layout"),
            bind_group_layouts: &[Some(&layout)],
            immediate_size: 0,
        });
        let scene_pipeline = scene_pipeline(&device, &pipeline_layout, &shader, format);
        let postprocess_pipeline = postprocess_pipeline(&device, &pipeline_layout, &shader, format);
        let targets = create_targets(&device, &config);
    ...
    fn resize(&mut self, width: u32, height: u32) {
        ...
        self.targets = create_targets(&self.device, &self.config);
    ...
    fn render(&mut self, seconds: f32) -> Result<(), Box<dyn Error>> {
        if !self.configured {
            return Ok(());
        }
        ...
        self.queue.write_buffer(
            &self.uniform_buffer,
            0,
            bytemuck::cast_slice(&scene::transform(seconds, aspect)),
        );
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("postprocess cube frame"),
            });
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("offscreen scene pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: &self.targets.multisample,
                    depth_slice: None,
                    resolve_target: Some(&self.targets.scene_color),
                    ops: wgpu::Operations {
...
fn bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("postprocess cube bindings"),
        entries: &[
...
fn create_targets(device: &wgpu::Device, config: &wgpu::SurfaceConfiguration) -> Targets {
    let descriptor = |label, format, sample_count, usage| wgpu::TextureDescriptor {
        label: Some(label),
        size: wgpu::Extent3d {
            width: config.width,
    ...
    let multisample = device.create_texture(&descriptor(
    ...
    Targets {
        ...
        multisample: multisample.create_view(&wgpu::TextureViewDescriptor::default()),
    }
}
