...
| Compose multiple passes through intermediate textures | Shadow, main MSAA, and fullscreen post-process passes share private resources within one command buffer. The extracted two-pass checkpoint also physically rendered resolved scene color into the fullscreen pass under Metal validation on the Apple M2 tier. | A depth-only shadow pass transitions its persistent map for scene sampling; the scene then renders into a resize-dependent offscreen target and feeds a fullscreen post-process pass into the swapchain. The extracted peer now implements the narrower scene-color transition and compiles/lints for Win32, but lacks a physical Vulkan run. | Intermediate attachments are ordinary resources or frame-scoped transient resources with explicit pass-to-pass dependencies. Presentation images are not required to be directly renderable by the game. | Automatic transient allocation, aliasing, arbitrary pass ordering, and physical Vulkan evidence for the extracted two-pass path are pending; the API must not promise a full frame graph from one fixed sequence. |
