//! Minimal fixed simulation state, collision, and win/reset loop.

use glam::Vec2;
use mulciber_platform::KeyCode;
use mulciber_runtime::InputSnapshot;
...
impl Game {
    pub fn handle_frame_input(&mut self, input: &InputSnapshot) {
    ...
    pub fn fixed_update(&mut self, input: &InputSnapshot, delta_seconds: f32) {
    ...
    fn check_hazards(&mut self) {
        if Self::hazards_at(self.current.world_seconds)
            .iter()
            .any(|hazard| {
                self.current.player.distance_squared(*hazard) < HAZARD_RADIUS * HAZARD_RADIUS
            })
        {
...
fn direction(input: &InputSnapshot) -> Vec2 {
    Vec2::new(
        f32::from(input.key_held(KeyCode::KeyD) || input.key_held(KeyCode::ArrowRight))
            - f32::from(input.key_held(KeyCode::KeyA) || input.key_held(KeyCode::ArrowLeft)),
        f32::from(input.key_held(KeyCode::KeyS) || input.key_held(KeyCode::ArrowDown))
            - f32::from(input.key_held(KeyCode::KeyW) || input.key_held(KeyCode::ArrowUp)),
    )
    .normalize_or_zero()
}
...
mod tests {
    ...
    fn collecting_a_crystal_removes_it_from_the_render_set() {
        ...
        game.fixed_update(&InputSnapshot::default(), 0.0);
