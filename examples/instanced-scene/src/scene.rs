//! Geometry, texture, and grouped transform data for the instanced scene comparison.
...
pub fn instance_transforms(seconds: f32, aspect: f32) -> [Vec<[[f32; 4]; 4]>; 4] {
    ...
    let mut batches: [Vec<[[f32; 4]; 4]>; 4] = std::array::from_fn(|_| Vec::with_capacity(34));
    for z in 0..DEPTH {
        for x in 0..WIDTH {
            let index = z * WIDTH + x;
            ...
            let mesh = index % 2;
            let texture = usize::from(index.is_multiple_of(3));
            batches[mesh * 2 + texture].push((projection * view * model).to_cols_array_2d());
        }
    }
    batches
}
