...
| Reclaim public resources before session shutdown | The extracted slice uses owning non-`Copy` handles, explicit fallible destruction, drop-queued reclamation, and generational slot reuse. Metal releases session retains while retained command buffers preserve submitted uses; all seven current resource kinds, including the instanced pipeline, and replacement rendering passed Metal API Validation on the Apple M2 tier. **Established** for the current Metal slice. | The same owning API and generational arenas run on Vulkan. Destruction waits the one in-flight frame fence and rebuilds descriptor pools that referenced destroyed textures or postprocess targets. The six pre-instancing resource kinds, drop-driven generational slot reuse, and replacement rendering passed the asserted conformance probe under `VK_LAYER_KHRONOS_validation` on the Intel UHD 620 tier with zero validation or loader messages. The seventh instanced-pipeline kind compiles but lacks physical Vulkan reclamation evidence. **Partial** for the current Vulkan slice. | Explicit destruction reports failure; `Drop` queues safe reclamation; shutdown destroys anything remaining. Reused slots never make an old identity refer to a replacement. | Multiple queues, multiple frames in flight, bindless tables, external ownership, and allocator integration require a broader lifetime model. |
