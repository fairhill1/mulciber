//! Selects the postprocess shader artifact shared by the showcase cube.
...
fn main() {
    ...
    let artifact = if target_os == OsStr::new("macos") {
        if std::env::var_os("MULCIBER_METAL_TYPECHECK_ONLY").as_deref() == Some(OsStr::new("1")) {
            ...
            fs::write(Path::new(&output).join("postprocess.shaderbin"), [])
                .expect("create cross-host shader placeholder");
            println!("cargo::warning=Metal showcase artifact skipped for cross-host type checking");
            return;
        }
        ...
        panic!("the showcase cube supports macOS, Windows, and Linux targets");
    };
    println!("cargo::rerun-if-changed={artifact}");
    let output = std::env::var_os("OUT_DIR").expect("Cargo sets OUT_DIR");
    fs::copy(artifact, Path::new(&output).join("postprocess.shaderbin"))
        .expect("copy cached showcase shader artifact");
}
