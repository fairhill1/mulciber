//! Gate 2 comparison implementation: the same textured spinning cube as `examples/cube`,
//! written as an ordinary best-practice `wgpu` + `winit` application.
//!
//! It shares the exact WGSL module and scene data with the Mulciber example, prints the same
//! observability lines (`surface configured at WxH`, `presented N textured cube frame(s)`), and
//! accepts the same validation-probe flags (`--frames N`, `--force-one-sample`) so the identical
//! KWin resize-storm harness can drive both implementations.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("presented {} textured cube frame(s)", app.presented);
    Ok(())
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attributes = Window::default_attributes()
            .with_title("wgpu — comparison cube validation")
            .with_inner_size(LogicalSize::new(960, 540));
        match event_loop.create_window(attributes) {
            Ok(window) => {
                let window = Arc::new(window);
                match pollster::block_on(Gpu::new(window.clone(), self.options.force_one_sample)) {
                    Ok(gpu) => {
                        println!(
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                match gpu.render(self.started.elapsed().as_secs_f32()) {
                    Ok(true) => {
                        self.presented += 1;
                        if self
                            .options
...
impl Gpu {
    async fn new(window: Arc<Window>, force_one_sample: bool) -> Result<Self, Box<dyn Error>> {
        ...
        let sample_count = if force_one_sample { 1 } else { 4 };
    ...
    fn render(&mut self, seconds: f32) -> Result<bool, Box<dyn Error>> {
        if !self.configured {
            return Ok(false);
        }
