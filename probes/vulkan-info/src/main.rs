...
mod windows {
        ...
        fn load() -> Result<Self, ProbeError> {
            ...
            if library.is_null() {
                return Err(ProbeError(
                    "could not load vulkan-1.dll; install a Vulkan driver".into(),
                ));
            }
            ...
            let get_instance_proc_addr: vk::PFN_vkGetInstanceProcAddr = unsafe {
                cast_address(
                    GetProcAddress(library, c"vkGetInstanceProcAddr".as_ptr()),
    ...
    impl Drop for Entry {
        fn drop(&mut self) {
            // SAFETY: All Vulkan children are destroyed before the owning loader is dropped.
    ...
    impl Context {
        fn new(window: &Window) -> Result<Self, ProbeError> {
            ...
            for required in [c"VK_KHR_surface", c"VK_KHR_win32_surface"] {
            ...
            let enabled_extensions = [c"VK_KHR_surface".as_ptr(), c"VK_KHR_win32_surface".as_ptr()];
            ...
            check(
                // SAFETY: The hidden window and instance remain live; output is writable.
                unsafe {
                    functions.create_win32_surface.expect("loaded function")(
                        instance,
    ...
    impl AdapterReport {
        ...
        fn collect(context: &Context, device: vk::VkPhysicalDevice) -> Result<Self, ProbeError> {
            ...
            if !queues.iter().any(|queue| queue.graphics && queue.present) {
                baseline_failures
                    .push("no queue family supports both graphics and this Win32 surface".into());
            }
            if surface.formats.is_empty() {
                baseline_failures.push("the Win32 surface exposes no formats".into());
            }
            if !surface
                .present_modes
            ...
            {
                baseline_failures
                    .push("the Win32 surface does not expose FIFO presentation".into());
            }
            Ok(Self {
