//! Selects a cached native artifact generated from the cube's single WGSL source.
...
fn main() {
    ...
    let artifact = if target_os == OsStr::new("macos") {
        ...
        "artifacts/cube.metal.shaderbin"
    } else if target_os == OsStr::new("windows") || target_os == OsStr::new("linux") {
        "artifacts/cube.vulkan.shaderbin"
    } else {
        panic!("the cube checkpoint supports macOS, Windows, and Linux targets");
    };
