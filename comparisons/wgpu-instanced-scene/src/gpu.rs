...
impl Gpu {
    ...
    pub(crate) fn render(&mut self, seconds: f32) -> Result<(), Box<dyn Error>> {
        ...
        let transforms = scene::instance_transforms(seconds, aspect);
        let mut bytes = Vec::with_capacity(usize::try_from(OBJECT_COUNT * 64).expect("fits usize"));
        let mut ranges = [(0_u64, 0_u32); 4];
        for (index, batch) in transforms.iter().enumerate() {
            ranges[index] = (
    ...
    fn encode_scene(&self, encoder: &mut wgpu::CommandEncoder, ranges: &[(u64, u32); 4]) {
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("instanced scene pass"),
        ...
        for (batch, &(offset, count)) in ranges.iter().enumerate() {
            let mesh = &self.meshes[batch / 2];
            let texture = batch % 2;
            pass.set_bind_group(0, &self.scene_bind_groups[texture], &[]);
...
fn bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("instanced scene bindings"),
        entries: &[
