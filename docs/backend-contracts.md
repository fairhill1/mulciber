...
| Reclaim public resources before session shutdown | The extracted slice uses owning non-`Copy` handles, explicit fallible destruction, drop-queued reclamation, and generational slot reuse. Metal releases session retains while retained command buffers preserve submitted uses; all six resource kinds and replacement rendering passed Metal API Validation on the Apple M2 tier. **Established** for the current Metal slice. | The same owning API and generational arenas run on Vulkan. Destruction waits the one in-flight frame fence and rebuilds descriptor pools that referenced destroyed textures or postprocess targets. All six resource kinds, drop-driven generational slot reuse, and replacement rendering passed the asserted conformance probe under `VK_LAYER_KHRONOS_validation` on the Intel UHD 620 tier with zero validation or loader messages. **Established** for the current Vulkan slice. | Explicit destruction reports failure; `Drop` queues safe reclamation; shutdown destroys anything remaining. Reused slots never make an old identity refer to a replacement. | Multiple queues, multiple frames in flight, bindless tables, external ownership, and allocator integration require a broader lifetime model. |
...
| Submit a heterogeneous object sequence in one scene pass | The extracted scene operation keeps one render encoder open, uses checked 256-byte transform offsets, and switches pipeline, mesh, and texture per record. A 100-object visual smoke and direct/postprocessed two-object conformance cases passed Metal API Validation on the Apple M2 tier. **Established** for the current Metal slice. | The peer implementation records one dynamic-rendering pass, binds a dynamic uniform offset plus the selected resources per record, and issues ordered indirect indexed draws. It compiles and lints for Windows, but the new path has no physical Vulkan execution evidence. **Implemented, unverified physically.** | A non-empty ordered scene may select resources and one finite transform independently per object; all records belong to the frame session and one generation-bound scene pass. | GPU instancing, sorting, batching, material/binding generality, object-count limits, multiple frames in flight, and physical Vulkan evidence remain pending. |
