...
0.1), and the platform owns the startup wait for first drawable metrics. An AppKit-first experimental
slice now delivers ordered physical-key, modifier, pointer, scroll, and focus transitions; peer
Win32, Wayland, and X11 input implementations remain in progress, as do display enumeration,
multi-window support, and a stable application-facing API. The current API is research-stage and may
change without compatibility guarantees. On docs.rs, use the target selector to view the
implementation available for each operating system.
