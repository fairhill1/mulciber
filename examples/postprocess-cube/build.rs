//! Selects the cached native artifact generated from the postprocess cube's WGSL source.
...
fn main() {
    ...
    let artifact = if target_os == OsStr::new("macos") {
        if std::env::var_os("MULCIBER_METAL_TYPECHECK_ONLY").as_deref() == Some(OsStr::new("1")) {
            ...
            println!(
                "cargo::warning=Metal postprocess artifact skipped for cross-host type checking"
            );
            return;
        }
    ...
    fs::copy(artifact, Path::new(&output).join("postprocess.shaderbin"))
        .expect("copy cached postprocess shader artifact");
}
